...
// this will cause compile error if open DibitVector::allocator
// attribute [[no_unique_address]] under GCC10
// Open notation in the future while this bug fixed.
...
TEST_CASE("DibitVector::Operations - Performs various vector operations", "[DibitVector]") {
  ...
  SECTION("reserve") {
    ...
    // !!!undefined behavior !!!
    // > also, std::vector<bool>::shrink_to_fit()
    v.shrink_to_fit();
    // REQUIRE(v.capacity() < 1024);
  ...
  std::filesystem::create_directory(data_path / "tmp");

  SECTION("serialization") {
    DibitVector<> v(65536, 1);
    std::ofstream fout{data_path / "tmp" / "test.dbv"};
    Serializer::save(fout, v);
    fout.close();
    DibitVector<> v2;
    std::ifstream fin{data_path / "tmp" / "test.dbv"};
    REQUIRE(fin);
    Serializer::load(fin, v2);
    fin.close();
    REQUIRE(v == v2);
    std::filesystem::remove(data_path / "tmp" / "test.dbv");
  }

  SECTION("serialization std::vector<bool>") {
    std::vector<bool> v;
    std::default_random_engine gen;
    std::uniform_int_distribution<int> dis(0, 1);
    for (std::size_t i = 0; i < 1000; i++) v.push_back(dis(gen));
    std::ofstream fout{data_path / "tmp/vector_of_bool"};
    Serializer::save(fout, v);
    fout.close();
    std::vector<bool> v2;
    std::ifstream fin{data_path / "tmp/vector_of_bool"};
    Serializer::load(fin, v2);
    fin.close();
    REQUIRE(v == v2);
    std::filesystem::remove(data_path / "tmp/vector_of_bool");
  }
}