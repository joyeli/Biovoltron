...
TEST_CASE("BedSixReader") {
  ...
  SECTION("read from gencode gtf - main") {
    ...
    auto first = records.front();
    REQUIRE(first.seqid == "chr1");
    REQUIRE(first.start == 3073253);
    REQUIRE(first.end == 3074322);
    REQUIRE(first.strand == '+');
    REQUIRE(first.gene_type == "TEC");
    REQUIRE(first.gene_name == "4933401J01Rik");
    REQUIRE(Interval{first} == Interval{"chr1", 3073253, 3074322, '+'});

    auto last = records.back();
    REQUIRE(last.seqid == "chr2");
    REQUIRE(last.start == 58567298);
    REQUIRE(last.end == 58567528);
    REQUIRE(last.strand == '+');
    REQUIRE(last.gene_type == "protein_coding");
    REQUIRE(last.gene_name == "Upp2");
    REQUIRE(Interval{last} == Interval{"chr2", 58567298, 58567528, '+'});
  }
  ...
  SECTION("read from gencode gtf - polyAs") {
    ...
    read_gencode_gtf(polyA_path, records);
    auto first = records.front();
    REQUIRE(first.seqid == "chr1");
    REQUIRE(first.start == 3214481);
    REQUIRE(first.end == 3214482);
    REQUIRE(first.strand == '-');
    REQUIRE(first.gene_type == "polyA_site");
    REQUIRE(first.gene_name == "744347");
    REQUIRE(Interval{first} == Interval{"chr1", 3214481, 3214482, '-'});

    auto last = records.back();
    REQUIRE(last.seqid == "chr5");
    REQUIRE(last.start == 30674381);
    REQUIRE(last.end == 30674386);
    REQUIRE(last.strand == '+');
    REQUIRE(last.gene_type == "polyA_signal");
    REQUIRE(last.gene_name == "703510");
    REQUIRE(Interval{last} == Interval{"chr5", 30674381, 30674386, '+'});
  }
  ...
  SECTION("read from gencode gtf - tRNAs") {
    ...
    read_gencode_gtf(tRNAs_path, records);
    auto first = records.front();
    REQUIRE(first.seqid == "chr1");
    REQUIRE(first.start == 112349389);
    REQUIRE(first.end == 112349461);
    REQUIRE(first.strand == '+');
    REQUIRE(first.gene_type == "Pseudo_tRNA");
    REQUIRE(first.gene_name == "NULL");
    REQUIRE(Interval{first} == Interval{"chr1", 112349389, 112349461, '+'});

    auto last = records.back();
    REQUIRE(last.seqid == "chr11");
    REQUIRE(last.start == 113168794);
    REQUIRE(last.end == 113168861);
    REQUIRE(last.strand == '-');
    REQUIRE(last.gene_type == "Ala_tRNA");
    REQUIRE(last.gene_name == "NULL");
    REQUIRE(Interval{last} == Interval{"chr11", 113168794, 113168861, '-'});
  }
}
