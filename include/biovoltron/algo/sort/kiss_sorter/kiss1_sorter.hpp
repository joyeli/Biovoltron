#pragma once
#include <biovoltron/algo/sort/core/kiss1_core.hpp>
#include <vector>
...
struct KISS1Sorter {
  ...
  static auto
  get_sa(const std::ranges::random_access_range auto& ref,
        size_type k = 256u,
  ...
  static auto
  get_sa(const kiss::vector<uint8_t>& S, size_type k = 256u,
        const size_t num_threads = std::thread::hardware_concurrency()) {
    auto SA = kiss::vector<size_type>{};
