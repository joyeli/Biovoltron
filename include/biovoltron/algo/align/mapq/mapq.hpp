...
mem_mapq_pe(MemAln p0, MemAln p1, int score_un, int o, int subo, int n_sub) {
  ...
  if (q_pe > 60)
    q_pe = 60;
  q_pe =
    static_cast<int>(q_pe * (1. - .5 * (p0.frac_rep + p1.frac_rep)) + .499);
  ...
  q_se0 = q_se0 < raw_mapq(p0.score - p0.score2) ?
            q_se0 :
            raw_mapq(p0.score - p0.score2);
  q_se1 = q_se1 < raw_mapq(p1.score - p1.score2) ?
            q_se1 :
            raw_mapq(p1.score - p1.score2);
  if (q_se0 > 60)
    q_se0 = 60;
  if (q_se1 > 60)
    q_se1 = 60;
