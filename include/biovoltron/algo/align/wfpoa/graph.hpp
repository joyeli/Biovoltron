 ...
 public:
  ...
  struct Edge {
   public:
    ...
    void AddSequence(std::uint32_t label, std::uint32_t w) {
  ...
  std::vector<int> first_node() const{
    return first_node_;
  }
...
;
  ...
  void PrintGwfa(const std::string& path) const{
    ...
    for(const auto& it : nodes_){
      os << "S\tv" << std::to_string(it->id) << '\t' << static_cast<char>(decoder_[it->code]) << std::endl;
    }

    for (const auto& it : nodes_) {
      for (const auto& jt : it->outedges) {
        os << "L\tv" << std::to_string(it->id) << "\t+\tv" \
           << std::to_string(jt->head->id) << "\t+\t0M\n";
      }
    }
 ...
 private:
  ...
  std::vector<bool> ExtractSubgraph(const Node* begin, const Node* end) const {  // NOLINT
