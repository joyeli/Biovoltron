...
public:
  ...
  auto open(const std::filesystem::path& bam_path) {
    if (bam) clear();
    path = bam_path;
   ...
   */
  IBamStream(const std::filesystem::path& bam_path) {
    open(bam_path);
  };

  IBamStream(IBamStream&& other) noexcept {
    bam = other.bam;
    bam_header = other.bam_header;
    idx = other.idx;
    itr = other.itr;
    is_eof = other.is_eof;
    path = other.path;
    ...
    other.bam = nullptr;
    other.bam_header = nullptr;
    other.idx = nullptr;
    other.itr = nullptr;
    other.is_eof = false;
    other.itr_type = IterateType::SEQUENTIAL;
  }
  ...
  auto on_sequential() const noexcept {
    return itr_type == IterateType::SEQUENTIAL;
  };
  ...
  auto on_ranged() const noexcept { 
    return itr_type == IterateType::RANGED;
  };
  ...
  auto on_unmapped() const noexcept { 
    return itr_type == IterateType::UNMAPPED;
  };
...
public:
   ...
   */
  auto to_begin() { 
    return traverse_on(HTS_IDX_START, 0, 0, IterateType::SEQUENTIAL);
  };
   ...
   */
  auto load_index(const std::filesystem::path& index_path) {
 ...
 */
  friend auto& operator<<(OBamStream& os, SamHeader& h) {
    ...
    if (auto c_hdr_write = sam_hdr_write(os.bam, header); c_hdr_write < 0) { /* TODO: error handler */
      ...
      for (auto i = 0; i < header->n_targets; ++i) {
        os.ref_table[header->target_name[i]] = i;
      }
      // build idx
      if (os.write_idx) {
  ...
  friend auto& operator<<(OBamStream& os, SamRecord<Encoded>& r) {
    ...
    auto aln = bam_init1();
    os.bam_set1(aln, r, tid, cigars, quals, mtid);
   ...
   */
  auto open(const std::filesystem::path& bam_path, bool gen_idx = false) {
    if (bam) clear();
    path = bam_path;
   ...
   * @param gen_idx whether to generate a index file
   */
  OBamStream(const std::filesystem::path& path, bool gen_idx = false) { 
    open(path, gen_idx);
  };

  OBamStream(OBamStream&& other) noexcept {
    bam = other.bam;
    bam_header = other.bam_header;
    ref_table = std::move(other.ref_table);
    path = std::move(other.path);
    idx_path = std::move(other.idx_path);
    write_idx = other.write_idx;
    other.bam = nullptr;
    other.bam_header = nullptr;
    other.write_idx = false;
  }
...
private:
  ...
  auto convert_type_and_copy(U source, std::uint8_t* dest) {
  ...
  template<bool Encoded>
  auto aux_to_bin(bam1_t* aln, SamRecord<Encoded>& r) {
    auto& len = aln->l_data;
      ...
      case 'i': {
        if (opt[5] == '-') {
          ...
          if (num >= std::numeric_limits<std::int8_t>::min()) {
            aux_data[len++] = 'c';
            len += convert_type_and_copy<std::int8_t>(num, &aux_data[len]);
          } else if (num >= std::numeric_limits<std::int16_t>::min()) {
            aux_data[len++] = 's';
            len += convert_type_and_copy<std::int16_t>(num, &aux_data[len]);
          } else {
            aux_data[len++] = 'i';
            len += convert_type_and_copy<std::int32_t>(num, &aux_data[len]);
          }
        } else {
          auto num = static_cast<uint32_t>(std::stoul(opt.substr(5)));
          if (num <= std::numeric_limits<std::uint8_t>::max()) {
            aux_data[len++] = 'C';
            len += convert_type_and_copy<std::uint8_t>(num, &aux_data[len]);
          } else if (num <= std::numeric_limits<std::uint16_t>::max()) {
            aux_data[len++] = 'S';
            len += convert_type_and_copy<std::uint16_t>(num, &aux_data[len]);
          } else {
            aux_data[len++] = 'I';
            len += convert_type_and_copy<std::uint32_t>(num, &aux_data[len]);
          }
        }
        break;
      }
